...
public class RecurringSessionSteps
{
    ...
    {
        /*sessionsList.defaultSessionSearch(theo)
                .filter(sessionOneshot -> sessionOneshot.getSport().getName().equals(arg0))
                .filter(sessionOneshot -> sessionOneshot.getStart().isAfter(ZonedDateTime.now().plusDays(arg1+1))).forEach(sessionOneshot -> {
                    System.out.println(sessionOneshot.getMinParticipants() + " " + sessionOneshot.getStart());
                });*/
